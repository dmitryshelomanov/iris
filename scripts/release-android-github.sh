...
#   apk  = newest dist/android/iris-preview-*.apk, else newest build-*.apk
set -euo pipefail
...
find_apk() {
  local candidates=()
  shopt -s nullglob
  candidates+=(dist/android/iris-preview-*.apk)
...
TITLE="Iris ${TAG} (Android preview)"
NOTES="$(
  cat <<EOF
Sideload **preview** APK for Android (not a Play Store build).

Includes Anime ML looks (Shinkai / Hayao), look bake, and recent camera fixes.

\`\`\`bash
adb install -r ${ASSET_NAME}
\`\`\`

Build & store docs: [README](https://github.com/dmitryshelomanov/iris#android--google-play)
EOF
)"
